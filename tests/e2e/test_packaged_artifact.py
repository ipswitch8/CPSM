...
@pytest.mark.skipif(
    ...
    reason="Packaged dist/cpsm/cpsm not present; skipping build-stamp check.",
)
def test_packaged_binary_reports_a_real_build_stamp():
    """The packaged binary must name the commit it was built from.

    cpsm/_build_stamp.py is generated by packaging/cpsm.spec and imported
    by cpsm/__init__.py. That import has to stay STATIC: switching it to
    importlib.import_module satisfied a type checker but made PyInstaller's
    dependency analysis blind to the module, so it stopped being bundled
    and every packaged build silently degraded to "build unknown". The
    version number still looked right, so nothing else caught it.
    """
    ...
    assert "build unknown" not in combined, (
        f"packaged binary reports no build stamp: {combined!r}. "
        f"cpsm/_build_stamp.py was not bundled -- check that the import in "
        f"cpsm/__init__.py is still a static `from cpsm import _build_stamp`, "
        f"which is what PyInstaller's analysis needs to see."
    )
