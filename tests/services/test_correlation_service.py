...
class TestCorrelateService:
    def test_cwds_by_pid_populated_for_diagnostic_tooltip(self, tmp_path: Path) -> None:
        ...
        def fake(*_a: Any, **_kw: Any) -> str:
            return "CPSM_PROBE|1|50001|2|/var/log|tail\nCPSM_PROBE|3|50002|4|/etc|cat\n"
    ...
    def test_probe_home_threaded_to_path_compare(self, tmp_path: Path) -> None:
    ...
    def test_correlates_tilde_project_to_remote_root_home(self, tmp_path: Path) -> None:
        ...
        conn_a = _ssh_conn("conn-a", "10.0.0.1", project_folder="~/projects/foo")
        conn_b = _ssh_conn("conn-b", "10.0.0.1", project_folder="~/projects/bar")
        doc = CpsmDocument(ssh_keys=[_key()], connections=[conn_a, conn_b])
        sessions = [_disc_ssh(2001, "10.0.0.1"), _disc_ssh(2002, "10.0.0.1")]
    ...
    def test_handles_tilde_in_project_folder(self, tmp_path: Path) -> None:
        ...
        def fake(*_a: Any, **_kw: Any) -> str:
            return f"CPSM_PROBE|1|50001|2|{target}|claude\nCPSM_PROBE|3|50002|4|{tmp_path}|bash\n"
...
class TestProbeArgvPinsIdentity:
    ...
    def _capture(monkeypatch, key_path: str) -> list[str]:
        ...
        cs._run_probe_via_subprocess(host="h", user="u", key_path=key_path, port=22)
